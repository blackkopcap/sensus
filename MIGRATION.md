# Multi-User Support & Dynamic Reactions Migration
...
Теперь sensus может рекомендовать emoji-реакции вместо текстовых ответов, основываясь на:
- Эмоциональном состоянии агента
- Контексте сообщения (команда, благодарность, вопрос, шутка)
- WITHDRAWN состоянии (только angry реакции)
- Истории отношений с пользователем

**Новые файлы:**
- `reaction-config.json` — конфигурация системы реакций
- `scripts/reactions.js` — управление настройками реакций

**Новые команды:**
```bash
# Анализ предпочтений реакций
node limbic.js reaction-preference "спасибо!" --user user123

# Настройка системы реакций
...
**Интеграция с OpenClaw hook:**
Hook теперь выводит в stdout JSON с рекомендациями реакций, которые основной агент может использовать для выбора подходящих emoji вместо многословных ответов.
